...
def import_new_file(file_name):
    ...
    sql = "DELETE FROM physical_flows WHERE date_time >= %s AND date_time <= %s"
    cursor.execute(sql, (start_date, end_date))
    conn.commit()

    print("Importing ...", file_name)
    sql = "COPY physical_flows(date_time,resolution_code,out_area_code,out_area_type_code,out_area_name,out_map_code,in_area_code,in_area_type_code,in_area_name,in_map_code,flow_value,update_time) FROM '{}' DELIMITER '\t' CSV HEADER;".format(file_name)
    cursor.execute(sql)
    conn.commit()
