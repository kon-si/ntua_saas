...
async function getFlowsData(outMapCode, inMapCode, dateFrom, dateTo) {
    let data = await db.physical_flows.findAll({where: { out_map_code: outMapCode, in_map_code: inMapCode, date_time: {[Op.between]: [dateFrom, dateTo]} }});
    return data;
}

app.get("/:out_map_code/:in_map_code/:date_from/:date_to", async(req, res) => {
    try {
        // Set Headers
        const headers = {
        ...
        const data = await getFlowsData(out_map_code, in_map_code, date_from, date_to);
        ...
        const newClient = {
            id: clientId,
            in_map_code: in_map_code,
            out_map_code: out_map_code,
            date_from: date_from,
            date_to: date_to,
            response: res
        }
