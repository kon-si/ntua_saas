...
async function getGenerationData(mapCode, productionType, dateFrom, dateTo) {
    let data = await db.aggregated_generation.findAll({where: { map_code: mapCode, production_type: productionType, date_time: {[Op.between]: [dateFrom, dateTo]} }});
    return data;
}
...
app.get("/:map_code/:production_type/:date_from/:date_to", async(req, res) => {
    try {
        ...
        const data = await getGenerationData(map_code, production_type, date_from, date_to);
...
async function sendEventsToAll(dataFrom, dataTo) {
    ...
    for (const client of clients) {
        if (doOverlap(client.date_from, client.date_to, dataFrom, dataTo)) {
            const newData = await getGenerationData(client.map_code, client.production_type, client.date_from, client.date_to);
            const newResponse = {
                "Data": newData
            }
